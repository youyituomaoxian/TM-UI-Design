...
> **⚠️ 跨端守则（2026-08-13 更新）：移动端冻结已解除（2026-08-06 用户拍板，恢复可修改）。**
> 改 Web 若涉及移动端（共享 token / 组件体系 / validate-spec 对称改动 / 两端联动规则），
> **仍应先提醒用户拍板**，不静默改动两端联动资产。
...
- 布局决策 6 问（任务 / 优先级 / 关系 / 倾向 / 论证 / 高度；定义见 GENERATION-SOP 步骤 5 自查表）

---

## 章节索引（加载纪律 · 2026-09-10 新增）

> 全文 **约 47k 字符**——**别整份加载**。按当前任务只读命中章节（Grep 标题 / Read offset 取段），**[软] 章节必读、[硬] 章节按需检索**（门禁兜底）。

| 章 | 字符 | 何时读 |
|---|---|---|
| [硬] 0. B 端后台固定框架 | 7.2k | 判定是否套框架（判定必读） |
| [软] 1. 组件选择决策树 | 7.8k | 选组件时 · 必读 |
| [硬] 2. 禁止清单 | 1.0k | 生成前自查 · 简读 |
| [硬] 3. 组件状态机 | 7.5k | 写/改按钮·输入态时 |
| [硬] 4. 间距与布局速查 | 6.5k | 布局时 |
| [硬] 5. 生成检查清单 | 1.1k | 交付前 |
| [硬] 6. 布局黄金法则 | 1.8k | 布局时 · 必读 |
| [软] 7. 布局美学规范 | 5.9k | 排版/留白/层级 · 必读 |
| [硬] 8. 动效与交互 | 1.3k | 生成即携带 |
| [软] 9. 图表自建范式 | 4.3k | 含图表时（真源 CHART-SPEC.md） |

> 机器资产（`components.json` 71k / `template.css` 54k / `validate-spec.js` 107k）**不通读**——由门禁与构建消费；单点事实源见 `GENERATION-SOP.md §0`。
...
| 区域 | 高度/宽度 | 强制项 |
|---|---|---|
| 顶栏 Topbar | 高 **72** | 品牌蓝底、白字；**左：logo（高度 32 固定、宽度自适应、占位符待替换为项目 logo）+ 平台名 + 副标题**；**右：首页、用户名、修改密码、退出登录、全屏、暗色模式、语言**（暗色按钮 = 纯图标、与全屏/退出交互一致；语言 = 框架级多语言下拉，2026-08-12 拍板） |
| 侧边栏 Sidebar | 宽 **240**，可折叠至 **80** | 作业树/导航树（**无菜单分类标题，顶级节点直接并列**，2026-08-12 去分类），**第一层与第二层节点必须带图标**，第三层可不带；收起时只显示图标；**折叠按钮固定在右下角（不随菜单多少移动），作业树滚动条隐藏** |
| 内容区 Content | 自适应 | 卡片/表格/表单，沿用组件库 |
| 底栏 Footer | 高 **32** | 版权/系统信息**居中**显示 |

### [硬] 0.2 框架硬性规则（不可违反）

1. **顶栏结构固定**：从左到右必须是 `平台 logo` → `平台名称（大字）` + `副标题（小字）` → `spacer` → `首页` | `用户名` | `修改密码` | `退出登录` | `全屏` | `暗色模式` | `语言`（`.lang-sel` 下拉：简体中文/English 可切换，日本語/한국어/Deutsch/Français/Русский/Español 置灰待支持；框架级双语字典 data-i18n 切换 + localStorage `hx-lang` 记忆；2026-08-12 拍板）。不得擅自增加中间主导航或改变顺序。
...
8. **侧栏信息架构（2026-08-12 去菜单分类）**：平级一级模块**直接并列顶级 tree-node**（如 注塑车间 / 设备中心 / 运营中心 / 生产管理），**禁止 group-title 菜单分类标题**（克隆源已删，生成页不得新增）；禁止把平级模块嵌套成某中心的 `tree-node` 子项（层级错误 + 双导航根源）。
   - **结构契约（2026-08-04 补强，2026-08-12 去分类）**：全部顶级节点**收进【单一 `.tree` 滚动区】**（`<div class="tree">` 内直接放顶级 `<div class="tree-node">`），**禁止多个 `.tree` 并列**——`.sidebar .tree` 是 `flex:1 1 auto`（flex-grow），多个 `.tree` 会平分剩余空间撑大间距。
9. **作业树业务定制（2026-08-07 拍板）**：作业树**结构**是框架契约（层级≤3 / 一二级带图标 / 折叠 / 选中态 / **顶级节点并列、无 group-title 分类** / DOM 契约零改动）；**节点文本、分组、选中态是业务内容**——克隆 page-template 后按当前系统的模块结构定制（增删分组/节点、改文本、把 `.on` 选中态移到当前页所在节点），**禁止保留示例树**（示例树是制造业演示，不是通用骨架）。节点图标从 `icons/` 取（见 §7.9 图标守则）。
...
> **⚠️ 构图开放性（2026-08-12 用户拍板）**：**框架层（顶栏/侧栏/底栏）与组件层锁死，页面构图层开放**。本节「必含区块」保证内容完整性底线；**区块的排列 / 占比 / 构图由 Agent 按「布局决策 6 问」+「布局模式库」（GENERATION-SOP 步骤 5）自由选择**，禁把本节结构理解为唯一固定范式。门禁只保组件一致性 / 对齐 / 防溢出，不锁页面构图。
> **面包屑铁律（2026-08-12，2026-08-13 补层级）**：**有明确父级路径的页面必带面包屑条**（32 N5 底，RULES §6.1）；**面包屑层级必须与左侧作业树导航路径一致**（顶级节点 → 子级 → 当前页；去分类后**无「分类级」**——如「注塑车间 / 注塑机参数看板」，**禁把并列的顶级节点拼成父级**，如「设备中心 / 注塑车间」是错的）；仅顶级入口页（无父级，如运营总览 / 看板首页）可无——由「是否有父级路径」判定。
> 同框架多样化示范：`output/布局多样化测试/`（工单列表=顶筛+全宽表 / 设备监控看板=KPI 2×2+图告混排 / 生产分析=图表主导+侧摘要 / 设备详情=左右分栏）。
...
- **宽表列宽预算（V10 新增 2026-08-27，订单总览实战：11 列合计 1328px vs 容器 1120px 常态溢出）**：**≥8 列或含宽内容列（进度条/按钮组/双行单元格）的宽表必须 `.table--fixed` + 全部 th 显式 `width` 百分比（合计 = 100%）**，禁裸 `table-layout:auto` 放任 min-content 溢出；fixed 下长文本单元格配 `overflow:hidden;text-overflow:ellipsis;white-space:nowrap`（自造/工具类）；确需横向滚动的超宽表在 `.table-wrap` 内滚动并给表格 `min-width`。列宽预算方法：容器宽 ÷ 列数定基准，操作/勾选列收紧（≤10%），内容主列放宽。`.table--fixed` 用 `border-collapse:separate;border-spacing:0`（collapse + 百分比列宽在 Chromium 有幻影 2px scrollWidth）。**行内浮层菜单（操作 ⋯ 下拉）禁 absolute 展开**——overflow:auto 容器内会被撑宽出滚动条，用 **fixed 定位**（JS 按按钮坐标计算）或挂载到 body。**滚动条红线（显式化 2026-08-27，用户拍板；条款源头见常见问题表「表格超过 12 列」行）**：**≤12 列的表格必须在列宽预算内消化、禁止出现横向滚动条**（内容展示页滚动条破坏视觉统一），纵向同理（行数用分页控制，真源 minRow+pager）；仅 >12 列的超宽表允许 `.table-wrap` 横向滚动。**表格内徽章/标签组必须用真源 `.badge-row` 容器（gap 8，td 场景 margin 清零）**，禁裸 span 堆叠（无间距）；`.table td` 垂直 padding = 8（单行行高仍 44 不变，多行内容行高自适应时保上下呼吸）。**徽章收敛 +N（V10-B2 用户拍板 b，2026-08-27）**：窄列（≤10%）徽章超过 2 个时最多显 2 个 + +N 徽章（title 悬停看全，详情抽屉有完整清单）——行高统一双行节奏；选配窄列 badge 竖排（每 badge 宽 > 列内容宽）是 11 列预算下的信息密度物理形态，非缺陷。
...
#### 详情页（必含区块：基础信息 + 参数 + 日志）
- **布局变体**：A 纵向堆叠（面包屑→标题→键值对→参数表→时间轴，下为基准）/ B 左右分栏（左 7 信息 + 右 5 日志，示范：output/布局多样化测试/设备详情）/ C 页签分组。
...
#### 整页表单页（录入 / 编辑 · 多字段长表单，2026-08-13 补）
- **必含区块**：页头（标题 + 右上取消/保存按钮 `.page-actions`）+ 表单卡（字段分组）+ 底部操作区 `.form-actions`（右对齐：取消 `.btn-secondary` + 保存 `.btn-primary`）。
- 字段用 `.form-row`（label + input/sel/switch/textarea）；字段多时用 `.tabs` 分组收纳字段组（如基本信息 / 运行参数 / 保养设置）。
- **选型**：短表单（≤4 字段）用 Modal 382 弹窗；**长表单用整页表单页**（非 Modal，避免 382 宽不适配长表单）。
- 底部操作区 `.form-actions{margin-top:var(--space-base)}`（真源已定，与表单卡留间距）；`.page-actions` 放页头右上。

#### 看板 / 数据大屏（必含区块：KPI + 图表 + 表格，至少两区）
- **布局变体**：A 三区纵排（KPI 单行 → 图表 → 表格）/ B KPI 2×2 + 图告混排（示范：output/布局多样化测试/设备监控看板）/ C 图表主导 + 侧摘要（示范：output/布局多样化测试/生产分析）。三区骨架为一致性底线（§7.2），区内构图开放。
...
6. **卡片头右侧元素（软规则，2026-08-13 治本）**：`.card-head` 右侧的**状态标签/计数**用 `.card-head-meta`、**操作按钮**用 `.card-head-actions` 包裹（两者均 `margin-left:auto` 右对齐，与 card-body 内边距对齐）；**禁止裸放 `<span class="tag">` / 按钮在 `.card-head` 内贴左**（Agent 曾因 `.card-head-actions` 名不副实「操作」而漏用 → 状态标签裸放贴左）。
...
| `.breadcrumb` → `.page-head` | **16px**（`--space-base`，2026-08-12 由 LG 24 降档：面包屑与标题同属页头信息组，组内用 BASE 档） | `.breadcrumb{margin-bottom:var(--space-base)}` |
...
**A. 预算公式（随视口推导，禁硬编码）**：
`首屏预算 = 视口高 − 120`（topbar 72 + content 上下 padding 48；实测三方验证：768→648 / 900→796 / 1080→960）。规划页面 IA 时按目标视口（默认 900）核算，且**下限校验**：预算 < 648（vh<768）时视为小屏，首屏只保留 KPI 行 + 单卡。

**B. 页面类型 × 首屏构成矩阵**（「核心内容」按 RULES §1.1b 页面类型定义）：

| 页面类型 | 首屏核心构成 | 次屏内容 |
|---|---|---|
| 看板页 | KPI 行 + 主图表行 | 明细表格、告警中心、次级图表 |
| 列表页 | 筛选栏 + 表格首屏行数 | 表格余行（分页控制）、批量操作 |
| 详情页 | 基础信息卡全量 | 参数表、日志时间轴 |
| 表单页 | 整个表单（短表单一屏） | — |

**C. 首屏行规则**：① 首屏行**禁叠加行**（一列纵向叠 ≥2 卡的组合，自然高叠加必超预算——如「机型分布 + 预警中心」列）；② 核心图表行高 = 预算剩余（`chart-box--flex` 弹性吸收，与 V8b 闭环：**预算线为 flex 提供确定空间，弹性在预算内消化**）；③ 叠加卡内容若须首屏，压缩各自内容或拆分到次屏。

**D. `minRow` 与预算冲突裁决**：剩余预算 < 表格最小占位（表头 48 + `minRow`×44 + 分页 44 ≈ 444）时，表格数据行数 = `min(minRow, 剩余预算可容纳数)`（≥4），或整体移次屏；禁让表格卡被底边切半。

**E. 超预算裁决顺序（固定优先级，依次执行直到达标）**：① 精简头部区（通知行并入页头徽标、KPI 多行并单行）；② 首屏内容收敛（badge "+N" 折叠、次要素砍除）；③ 内容块移次屏；④ 接受滚动（末选，交付说明中标注「首屏含被切区块」）。

**F. 自查（每次交付必做）**：视口 900 下截图，确认切点落在区块 gap；核心区块完整。Playwright 运行时门禁「首屏截断检查」为二期立项（T5）。

**G. 生效范围**：新页面即日生效；存量 output 页面不强制回溯（重构时顺手对齐）。

**落地示范**：`output/订单总览_20260827_1006_分享版.html`（V10 适配：通知行删除、预警中心移次屏、图表行压进预算，切点落在区块 gap）。
...
1. 页面生成 → `headless` 截图（**`--timeout=3500` 真实等待动画播完**，窗口 ≥1440——`--virtual-time-budget` 不驱动 CSS animation，图表会空白）
