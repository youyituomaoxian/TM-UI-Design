...
├── audit-rules.json                # 审计规则真源 v1.4.1（machine M-01~06 / runtime R-01~07 / human H-01~07 + 迁移维度 D-1~D-12 + 豁免登记 + issue_reporting 设计系统问题报告制度）
...
```
打开 USAGE.html → 搜索「看板」→ 点「看板 / 数据大屏」卡片 → 复制「日常版」→
用弘讯设计系统生成「<看板描述>，实时数据 2 秒刷新 + 暂停开关」页面。
```

把 `<看板描述>` 换成你的需求（如「注塑机运行看板，含 KPI 图标卡 + 趋势/柱状图 + 设备表格 + 告警列表」）发给 AI 即按规范生成（框架 / 图标 / 图表 / 门禁全自动）。若你的 AI 是网页版等不加载 AGENTS.md 的工具，改用卡片里的「完整版」（含脚手架命令 + 规范路径 + 0 HIGH 门禁）。
