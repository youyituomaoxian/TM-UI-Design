...
1. `<仓库根>/GENERATION-SOP.md` — **流程总纲**：§① 生成主流程（选端 / B 端固定框架判定 / 页面选型 §1.1b / 克隆模板 / 产出落盘 / 图表 / 配色引擎 / 禁区）；**§② 存量项目改造 SOP**（审查修改已有页面：audit-spec 清零硬违规 + audit-rules.json migration_dimensions D-1~D-12 逐维对照 → 映射决策 → 分批修复 → 收口）。
2. 端目录规范真源（两端同构）：
...
- 存量改造走 GENERATION-SOP §②：审查目的 = 找出视觉不符合弘讯的地方并改成符合；**先台账后修复**（audit-spec 清零硬违规 + audit-rules.json migration_dimensions D-1~D-12 逐维对照），**P1 映射决策**（每个差异按 D 层 map_to 定「改成什么」），每批门禁回归；不主动 git commit/push
