#!/usr/bin/env node
/* ============================================================
 * new-page-mobile.js — 移动端页面脚手架（2026-08-07，P2 克隆来源标记）
 *
 * 用法：node new-page-mobile.js <语义名> [目标目录，默认 cwd/output]
 *
 * 行为：
 *   1. cp 弘讯移动端design-system/page-template.html → <目标>/output/<语义名>_<YYYYMMDD>_<HHmm>.html
 *   2. 修正 CSS link（template.css → 相对仓库端目录的相对路径）
 *   3. 注入 <meta name="x-template-clone" content="弘讯移动端design-system/page-template.html">
 *      —— P1 门禁 template.clone.missing 的克隆凭证
 *   4. 同目录同名自动追加 _2/_3（绝不覆盖）
 *
 * 铁律：page-template.html 是唯一框架来源（375×812 手机壳）；禁止自搭框架。
 * ============================================================ */
'use strict';
const fs = require('fs');
...
function main() {
  ...
  if (!name || !/^[\u4e00-\u9fa5A-Za-z0-9_-]+$/.test(name)) {
    console.error('用法: node new-page-mobile.js <语义名> [目标目录]（语义名：中文/字母/数字/-/_）');
    process.exit(1);
  }
  ...
  console.log(`   下一步: 在 <div class="page-view on"> 内按业务填内容（框架外壳零改动）。`);
}
