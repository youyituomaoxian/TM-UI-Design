...
- **纵向占满 + 对称（2026-08-24 数值化）**：折线数据 y 范围纵向覆盖 **≥ viewBox 高度 70%**（数据带不得集中一条带），上下留白对称（**上/下差 ≤ 高度 15%**）。已纳入门禁 `chart.line.vertical`（MED）——不达标即提示重映射 y（如 viewBox 高 100 → y≈[92,8]，上下对称占满）。
...
- **图表卡不强制加摘要行**（拍板）；卡片拉伸空白由图表区弹性吸收——弹性填充型用 `.chart-box--flex`（2026-08-24 取代原全局 `.card:has(.chart-box) .chart-box{flex:1;min-height:320px}`；仅 `.chart-box--flex` 被 :has 弹性拉高，环形/迷你图 `.chart-box--ring` 定高不再被拉伸）。
...
- **容器高度契约（2026-08-24 明确，防跨端误套）**：移动端 `.chart-box` 高度**固定值**（默认 `180px`、`.chart-box--sm` `120px`，移动小屏 4px 网格）；Web 端高度走 `--chart-height` token（默认 320px）+ 弹性填充用 `.chart-box--flex`。**两端高度体系不同（移动固定 / Web token），Agent 不得跨端互套**（如把 Web 的 320px 套到移动、或把移动 180px 当 Web 默认）。
