# 弘讯B端视觉系统 · 移动端设计系统更新日志（归档）

> 本文件为 `CHANGELOG.md` 滚动归档（2026-09-22 建立，加载纪律 SSOT A-2）：主文件只保留近期条目，更早条目**原样移入**此处，未做任何内容修改。
> 切割线（滚动更新）：mobile 归档 `[1.14.X]` 及更早。新条目一律写主文件顶部；超限时由 roll-changelog.js 自动滚动。

## [1.14.13] — 2026-09-10 · 设计系统问题报告制度（单向报告流全量泛化）

> 背景：同 Web 端 [1.9.26]——应用侧发现的设计系统问题（生成中/改造/使用中，不止审计漏检）应制度化记录并可返还维护者。audit-rules.json v1.2.1 新增根级 `issue_reporting`：报告文件 output/设计系统问题_*.md、六要素模板（现象/根因/归因/建议归层/建议方案/严重级）、严重级反馈时机（P0 即时 / P1 攒批 / P2 可选）、维护侧消费义务、散落『上报』条款收口声明；change_policy 泛化（漏检为 machine 层特例，执行问题不进报告）。GENERATION-SOP 新增「生成中发现设计系统问题的记录义务」节（§①/§② 通用）。

---

---
## [1.14.12] — 2026-09-10 · 漏检回灌（11242 报告）：审计体系 v1.2.0（M-06 + R-05~07/H-06~07 + 单向报告流）

> 背景：同 Web 端 [1.9.25]——维护侧消费应用侧漏检报告（11242 迁移审计 7 单）。归因：规则/制度/流程缺口（设计系统体系问题），无执行缺口。audit-rules.json v1.2.0：M-01 渐变色标点扩展（白字豁免在色标点内不适用）+ data-audit-exempt 机制补登；新增 M-06 引用 CSS 资产扫描（冻结 dist 降 MEDIUM 清单 / 自维护 HIGH）；新增 R-05 滚动中间态 / R-06 有数据态 / R-07 图片可辨识度；新增 H-06 逐 tab×逐皮×滚动态审查矩阵 / H-07 UX 语义兜底；change_policy 修正为单向报告流（应用侧只报告，维护侧回灌，feedback_log 带 report_source）。audit-spec.js 已实现 M-01 扩展 + M-06（接入 ci-local 阻断段）。GENERATION-SOP §② 铁律 5（遮挡类结构禁当装饰删除）+ 布局改动专项回归清单 + P3 审查矩阵升级。

---

---
## [1.14.11] — 2026-09-10 · 审计语义 v2（迁移维度 D-1~D-12 + 归因分场景）

> 背景：同 Web 端 [1.9.24]——被审查项目多为外部项目，归因二分法失效；「全」靠结构对偶保证。audit-rules.json v1.1.0 新增 migration_dimensions D-1~D-12（框架/色彩/字阶/组件/间距栅格/圆角投影密度/动效/图标/图表/状态语义/暗色/审美，每维 check + map_to + ref）+ change_policy 双向对账元规则；P1 改「映射决策」（每个差异定「改成什么」）。GENERATION-SOP §② 与全套使用文档同步。

---

---
## [1.14.10] — 2026-09-10 · 双端真源一致性机器门禁（check-sync）

> 背景：同 Web 端 [1.9.23]——双端 template.css 同步靠人肉，缺机器看护（设计系统问题）。仓库根新增 `check-sync.js` 并接入 ci-local 阻断段（基线 98→99 pass / 0 fail）：vendor 同构段逐声明一致 + 双端同名类（33 个交集）属性键差集白名单比对。Mobile 侧：vendor `components.css` 为 Taro 组件库自有命名空间（46 类 `.mn-*`，与真源零交集），信息性清点；15 类同名漂移全为平台合法差异（Mobile 字阶上提父容器继承等），已登记 `SYNC_WHITELIST`。

---

---
## [1.14.9] — 2026-09-10 · 真源治理契约（分级治理）

> 背景：同 Web 端 [1.9.22]——用户拍板「分级治理」，RULES.md 新增「真源治理契约」段：使用者只读消费（css.override 门禁兜底）/ 维护者六步流程留痕（登记→改→双端同步→门禁全绿→刷指纹→CHANGELOG）/ 冻结区改动即 HIGH。角色由行为定义：改真源区文件即进入维护者流程；Agent 默认仅限使用者区，改真源须用户显式下发任务。

---

---
## [1.14.8] — 2026-08-25 · 门禁加固 + 文档图标数对齐

### Changed
- **P0-1 文档图标计数门禁（治文档漂移，与 Web 同步）**：ci-local.js 新增 `iconCountAssertions()`——解析 MASTER/RULES 的「图标库/图标集/Icon … N 个」声明 vs `icons/` 目录实际，漂移即 fail。
- **P0-2 RED-003 升级为阻断（与 Web 同步）**：根目录 *.html 由信息性升级为硬门禁；output/ 历史归档扫描保持信息性不阻断（分层决策：历史产物非新交付标准）。
- **文档数字对齐**：RULES.md 图标库「与 Web 同源 70 个通用」→「184 个（2026-08-14 换新后，索引见 icons/icons.md）」；MASTER.md 组件清单「图标集 ~30+」→「184 个（以 icons/ 目录为准）」（门禁正则不匹配「N+」格式，本次手动对齐）。
- **验证**：ci-local 全绿（含 P0-1 六项断言 + RED-003 阻断一项，基线 86→93 pass）。
