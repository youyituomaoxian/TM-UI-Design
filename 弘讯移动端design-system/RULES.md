...
> 全文 **约 49k 字符**——**别整份加载**。按当前任务只读命中章节（Grep 标题 / Read offset 取段），**[软] 章节必读、[硬] 章节按需检索**（门禁兜底）。

| 章 | 字符 | 何时读 |
|---|---|---|
| [软] 1. 组件选择决策树 | 6.3k | 选组件时 · 必读 |
| [硬] 2. 禁止清单 | 1.1k | 生成前自查 · 简读 |
| [硬] 3. 组件状态机 | 5.5k | 写/改按钮·输入态时 |
| [硬] 4. 间距与布局速查 | 5.6k | 布局时 |
| [硬] 5. 平台差异对照表 | 1.4k | 跨端对齐时 |
| [硬] 6. 数据字段映射表 | 1.7k | 字段落位时 |
| [硬] 7. 生成检查清单 | 1.1k | 交付前 |
| [硬] 8. 屏幕自动布局黄金法则 | 2.3k | 防溢出 · 必读 |
| [硬] 9. 组件体系规格 | 6.8k | 组件细节 · 必读 |
| [软] 10. 布局美学规范 | 9.7k | 排版/留白/层级 · 必读 |
| [硬] 11. 组件级样例表 | 5.0k | 观感基准 |

> 机器资产（`components.json` 41k / `template.css` 59k / `validate-spec.js` 64k）**不通读**——由门禁与构建消费；单点事实源见 `GENERATION-SOP.md §0`。
...
> **⚠️ 构图开放性（2026-08-12 用户拍板，与 Web 对称）**：**框架层（顶部导航/底部导航/home indicator）与组件层锁死，内容区构图开放**。本节「必含区块」保证内容完整性底线；**区块排列/顺序/取舍由 Agent 按「布局决策 6 问」+「布局模式库」（GENERATION-SOP 步骤 5）自由选择**（移动端受 343 列宽 + gap 12 间距体系约束）。门禁只保组件一致性与防溢出，不锁构图。
> **预览壳亮暗切换（2026-08-12 拍板）**：HTML 预览的 `.theme-toggle` 亮/暗切换按钮位于**手机框外**（`.stage-wrap` 顶部、与框右缘对齐），非页面内容——真实移动端无此按钮，克隆后勿删（预览壳层自带，localStorage `hx-theme` 记忆）。
...
> 移动端页面（尤其首页/看板）按**区域划分**组织内容。**§9.6 顺序为「可选区域 + 顺序按业务可调」（2026-08-12 拍板）**：下方自上而下为基准顺序（运营首页范式），Agent 按「布局决策 6 问 + 布局模式库」（GENERATION-SOP 步骤 5）调整区域顺序与取舍——例如告警优先页 = 列表区提前 + 筛选 Tab 置顶；工具页 = 金刚区置顶。铁律（不可违反）见节末：首屏 ≥3 区域（列表必带）、导航栏透明 StatusBar、区域间距 16px。
...
1. 页面生成 → `headless` 截图（**`--timeout=3500` 真实等待动画播完**——`--virtual-time-budget` 不驱动 CSS animation，图表会空白）
...
| 8 | **审美优先于对比度**（08-05 拍板）| WCAG 2.0 仅参考 | 功能色标签/按钮**白字首选**（视觉通透，深字方案已废弃，见 §10.3b）；WCAG 对比度作参考基线不作硬门禁；纯色不得单独传达信息 |
