...
| 文件名 | 图标 | 用途 | 类目 |
|---|---|---|---|
| `back.svg` | 返回 | 导航栏返回（chevron-left） | 通用导航 |
| `home.svg` | 首页 | 底部导航/金刚区首页入口 | 通用导航 |
| `search.svg` | 搜索 | 搜索框、筛选入口 | 通用导航 |
| `message.svg` | 消息 | 底部导航/告警消息入口 | 通用导航 |
| `user.svg` | 我的 | 个人中心/操作员 | 通用导航 |
| `settings.svg` | 设置 | 参数配置入口 | 通用导航 |
| `more.svg` | 更多 | 溢出操作（水平三点） | 通用导航 |
| `close.svg` | 关闭 | 弹窗/通知关闭 | 通用导航 |
| `plus.svg` | 新增 | 添加入口 | 通用导航 |
...
| `dryer_loader.svg` | 干燥上料机 | 干燥机/上料机辅机 | 设备 |
| `mold.svg` | 模具 | 模具管理/换模 | 设备 |
| `motor.svg` | 电机泵 | 液压泵、伺服电机 | 设备 |
| `temperature.svg` | 温度 | 温度监控（温度计） | 设备 |
| `pressure.svg` | 压力 | 压力监控（压力表） | 设备 |
| `running.svg` | 运行 | 运行中（语义绿） | 状态告警 |
| `stopped.svg` | 停机 | 已停机（灰） | 状态告警 |
| `fault.svg` | 故障 | 故障/告警（语义红） | 状态告警 |
| `warning.svg` | 警告 | 预警（语义黄） | 状态告警 |
| `maintenance.svg` | 维护 | 保养/维护（扳手） | 状态告警 |
| `offline.svg` | 离线 | 设备离线（断开） | 状态告警 |
| `alarm_bell.svg` | 告警铃 | 告警推送 | 状态告警 |
| `e_stop.svg` | 急停 E-STOP | 急停按钮（八角+圆） | 安全 |
| `lock.svg` | 安全锁 | 设备安全锁/权限 | 安全 |
| `shield.svg` | 防护盾 | 安全防护/权限保护 | 安全 |
| `safe_check.svg` | 安全确认 | 安全确认/复核通过 | 安全 |
| `report_work.svg` | 报工 | 生产报工 | 业务-生产 |
| `work_order.svg` | 工单 | 生产工单 | 业务-生产 |
| `production.svg` | 产量统计 | 产量柱状统计 | 业务-生产 |
| `quality.svg` | 质检 | 质检/良品检查 | 业务-质量 |
| `formula.svg` | 配方 | 配方管理 | 业务-工艺 |
| `process_param.svg` | 工艺参数 | 工艺参数（滑杆） | 业务-工艺 |
| `energy.svg` | 能耗 | 能耗/用电 | 业务-能耗 |
| `trend.svg` | 趋势图 | 数据趋势 | 业务-数据 |

## P1/P2 业务图标（30）

| 文件名 | 图标 | 用途 | 类目 |
|---|---|---|---|
| `production_plan.svg` | 生产计划 | 生产排产计划（日历+勾） | 业务-生产 |
| `schedule.svg` | 排产任务 | 排产甘特任务条 | 业务-生产 |
| `yield.svg` | 合格率 | 良品率/合格率（环+%） | 业务-生产 |
| `cycle_time.svg` | 周期时间 | 成型周期计时 | 业务-生产 |
| `oee.svg` | OEE 稼动率 | 设备综合效率仪表 | 业务-生产 |
| `downtime.svg` | 停机记录 | 停机时间记录（时钟+断开） | 业务-生产 |
| `order.svg` | 订单 | 生产订单 | 业务-生产 |
| `temp_zone.svg` | 温度区段 | 料筒温度区段曲线 | 业务-工艺 |
| `inject_pressure.svg` | 注射压力 | 注射单元/注射压力 | 业务-工艺 |
| `hold_pressure.svg` | 保压 | 保压/稳压（圆+双线） | 业务-工艺 |
| `clamp_force.svg` | 锁模力 | 合模机构/锁模力 | 业务-工艺 |
| `param_compare.svg` | 参数对比 | 工艺参数对比（双滑杆） | 业务-工艺 |
| `load_param.svg` | 工艺调取 | 从配方库调取工艺（文件夹+下载） | 业务-工艺 |
| `measure.svg` | 尺寸测量 | 卡尺测量 | 业务-质量 |
| `defect.svg` | 缺陷记录 | 缺陷/不良标记 | 业务-质量 |
| `spc.svg` | SPC 统计 | 统计过程控制（钟形+控制线） | 业务-质量 |
| `measure_report.svg` | 测量报告 | 测量/检验报告 | 业务-质量 |
| `meter.svg` | 电表 | 电能表（方表） | 业务-能耗 |
| `carbon.svg` | 碳排放 | 碳排放（叶子） | 业务-能耗 |
| `energy_save.svg` | 节能 | 节能灯/节能模式 | 业务-能耗 |
| `check_list.svg` | 点检 | 设备点检清单（双勾） | 业务-维护 |
| `maintain_plan.svg` | 保养计划 | 保养计划（日历+勾） | 业务-维护 |
| `lubricate.svg` | 润滑 | 润滑加油（油滴） | 业务-维护 |
| `wear_part.svg` | 易损件 | 易损件/备件（齿轮） | 业务-维护 |
| `spare_parts.svg` | 备件更换 | 备件更换（交换箭头） | 业务-维护 |
| `report_chart.svg` | 报表 | 统计报表（文档+柱） | 业务-数据 |
| `export.svg` | 导出 | 数据导出（下载） | 业务-数据 |
| `data_collect.svg` | 数据采集 | 数据采集（数据库） | 业务-数据 |
| `remote.svg` | 远程监控 | 远程监控（显示器） | 业务-数据 |
| `cloud.svg` | 云端 | 云端/云平台 | 业务-数据 |
| `click.svg` | 点击手势 | 交互点击/按压手势（食指） | 交互 |
