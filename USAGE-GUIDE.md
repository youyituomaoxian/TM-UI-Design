...
| 质量闸 | HTML 预览产物 → `validate-spec.js` **0 HIGH**；参考实现产物 → `validate-static.js` 全绿；总门禁 `node ci-local.js` → **全绿 0 fail**（2026-09-10 起；项数以输出为准） |
...
### Agent 读取顺序（2026-09-10 起 · 加载纪律见 `GENERATION-SOP.md §0`）

**加载预算 ≤35k 字符/任务——只读命中章节，禁止整份加载。**

| 顺序 | 读什么 | 取什么 |
|------|------|--------|
| 1 | `GENERATION-SOP.md` §①（存量改造读 §②） | 生成/改造流程 |
| 2 | 对应端 `RULES.md` **命中章节**（按页首「章节索引」挑） | 组件选型 + 页面类型规格 §1.1b + 相关 [硬]/[软] 规则 |
| 3 | 对应端 `tokens.json` 的 `colors` 段 | 色板值（需要时再取字阶/间距） |

**不要通读**：`components.json`（组件规格机器契约）· `template.css` · `validate-spec.js` · `ci-local.js` —— 由门禁与构建消费，读脚本输出即可；`MASTER.md` / `DESIGN-TOKENS.md` 按章取。
...
4. **总门禁**：`node ci-local.js` → 期望全绿 `0 fail`（2026-09-10 起；项数以输出为准）。
...
已有 HTML 页面/项目按弘讯规范改造（审计修复 / 存量迁移）——**不走生成流程**，走 `GENERATION-SOP.md` **§② 存量项目改造 SOP** 五阶段。审查目的：找出视觉不符合弘讯设计系统的地方，修改成符合弘讯设计系统的视觉。

1. **P0 盘点**：`node audit-spec.js <页面.html> --end web|mobile` 清零硬违规（裸 hex/按钮态/状态点等）+ 按 `audit-rules.json` migration_dimensions D-1~D-12 **逐维对照**（框架/色彩/字阶/组件/间距/动效/图标/图表/暗色/审美），差异全部落台账（禁止逐条手改式找问题）。
2. **P1 映射决策**：每个差异按 D 层 map_to 定「改成什么」——这个色映射到哪个 token、这个元素映射到哪个组件类、这个字号映射到哪级字阶。
3. **P2 修复**：分批修复，每批 `node ci-local.js` 回归全绿。
4. **P3 表现层**：截图审查运行时问题（截断/溢出/交互态/图表渲染）。
...
## 发现设计系统问题怎么办（2026-09-10 起 · 单向报告流）

用系统的过程中发现**设计系统本身**的问题（不是页面写法问题）：token/组件/图标缺口、规范歧义、门禁漏洞、文档缺陷——**AI 必须记录，你负责转交**：

1. **AI 先归因**：执行问题（规范写了没执行）→ 直接改页面，不记录；设计系统问题 → 按 `templates/设计系统问题报告模板.md` 落盘**你项目**的 `output/设计系统问题_YYYYMMDD_NN.md`；
2. **AI 会在对话里声明**：「这是设计系统 P? 问题，报告已落盘 `<路径>`，请转交维护者」；
3. **你把它转给维护者**即可——没有直连通道，也不需要 AI 主动打扰维护者（制度为**被动消费**）；
4. **反馈时机按严重级**：P0（真源错误/门禁漏洞/阻断生成）当场转交；P1（缺口/歧义）攒批（每 2 周或满 5 条）；P2（文档措辞）攒批可选；
5. **报告只有能被执行才有价值**——AI 需给出：定位（真源文件:行 / 规则 id / token 名）+ 最小复现片段 + 建议规则文本 + 建议回归样例（合格线三判据）。缺项时维护者会回问补齐。

> 制度真源：`audit-rules.json` → `issue_reporting`；模板：`templates/设计系统问题报告模板.md`。**AI 只报告、禁改真源**。
...
| `ci-local.js` | 全链路总门禁（含 audit 增量审计 + check-sync 一致性阻断；项数以输出为准） |
| `check-size.js` | 文档体积门禁（真源文件字符限额 + 清单封闭防膨胀转移，2026-09-22 起） |
...
**Q: 怎么确认没改坏？**
A: `node ci-local.js`，期望全绿 `0 fail`（2026-09-10 起；项数以输出为准）。
